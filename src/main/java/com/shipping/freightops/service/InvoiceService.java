...
public class InvoiceService {
    ...
    public byte[] generateInvoice(Long orderId) throws DocumentException, FileNotFoundException {
        FreightOrder order = freightOrderRepository.findById(orderId)
        ...
        header.addCell(getBorderlessCell("APGL SHIPPING LINE\nFreight Operations", FontFactory.getFont(FontFactory.HELVETICA, 14)));
        ...
        client.addCell(getBorderlessCell(order.getCustomer().getCompanyName(), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 11)));
        ...
        addRow(details, "Order Date", LocalDate.now().format(DATE_FMT), headBold, normal); // Remplacez par order.getCreatedAt() si dispo
        ...
        shipTable.addCell(new Phrase(order.getContainer().getContainerCode() + " (" +
                order.getContainer().getSize() + " " +
                order.getContainer().getType() + ")", normal));
        shipTable.addCell(new Phrase(order.getVoyage().getDeparturePort().getName() + " / " +
                order.getVoyage().getArrivalPort().getName(), normal));
        ...
        addPriceRow(priceTable, "Discount (" + order.getDiscountPercent() + "%)", "-$" + order.getBasePriceUsd().subtract(order.getFinalPrice()), normal, false);

        if (order.getDiscountReason() != null && !order.getDiscountReason().isEmpty()) {
            addPriceRow(priceTable, "Discount Reason", order.getDiscountReason(), FontFactory.getFont(FontFactory.HELVETICA_OBLIQUE, 9), false);
        }

        addPriceRow(priceTable, "FINAL PRICE (USD)", "$" + order.getFinalPrice(), FontFactory.getFont(FontFactory.HELVETICA_BOLD, 11, BRAND_TEAL), true);
    ...
    private void addRow(PdfPTable t, String l, String v, Font f1, Font f2) {
        PdfPCell c1 = getBorderlessCell(l + ":", f1); c1.setHorizontalAlignment(Element.ALIGN_RIGHT);
        t.addCell(c1);
        PdfPCell c2 = getBorderlessCell(v, f2); c2.setHorizontalAlignment(Element.ALIGN_RIGHT);
        t.addCell(c2);
    }
